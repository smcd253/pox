...
def ip_flow_mod(rt_object, dpid, packet):
  ...
  # DEBUG
  print("IP_FLOW_MOD(): Learning IP %s corresponds to MAC %s on PORT %d." % (str(packet.next.dstip), str(rt_object.ip_to_mac[dpid][str(packet.next.dstip)]), rt_object.ip_to_port[dpid][packet.next.dstip]))
...
def ipv4_handler(rt_object, dpid, packet, packet_in):
  ...
  if validate_ip(rt_object, dpid, str(packet.next.dstip)):
